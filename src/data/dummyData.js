export const bestPlaces = [
  {
    id: 1,
    name: "Batu, East Java",
    number_of_destinations: 86,
    image: `${process.env.PUBLIC_URL}/images/place (16).jpg`,
  },
  {
    id: 2,
    name: "Kuta",
    number_of_destinations: 46,
    image: `${process.env.PUBLIC_URL}/images/place (2).jpg`,
  },
  {
    id: 3,
    name: "Surabaya, East Java",
    number_of_destinations: 57,
    image: `${process.env.PUBLIC_URL}/images/place (3).jpg`,
  },
  {
    id: 4,
    name: "Meelang, east Java",
    number_of_destinations: 12,
    image: `${process.env.PUBLIC_URL}/images/place (16).jpg`,
  },
  {
    id: 5,
    name: "Dieng, Central Java",
    number_of_destinations: 76,
    image: `${process.env.PUBLIC_URL}/images/place (15).jpg`,
  },
  {
    id: 6,
    name: "Nusa Dua, Lombok",
    number_of_destinations: 42,
    image: `${process.env.PUBLIC_URL}/images/place (12).jpg`,
  },
  {
    id: 7,
    name: "Bandung, West Java",
    number_of_destinations: 41,
    image: `${process.env.PUBLIC_URL}/images/place (5).jpg`,
  },
  {
    id: 8,
    name: "Wakatobi, Sumatera",
    number_of_destinations: 28,
    image: `${process.env.PUBLIC_URL}/images/place (4).jpg`,
  },
];

export const featuredDestinations = [
  {
    id: 1,
    name: "Syri i Klatër",
    number_of_activities: 123,
    rating: 4.5,
    image: `${process.env.PUBLIC_URL}/images/syri-i-kalter.jpg`,
  },
  {
    id: 2,
    name: "Vlorë",
    number_of_activities: 353,
    rating: 4.5,
    image: `${process.env.PUBLIC_URL}/images/vlore.jpg`,
  },
  {
    id: 3,
    name: "Korçë",
    number_of_activities: 165,
    rating: 4.5,
    image: `${process.env.PUBLIC_URL}/images/Korce-Voskopoje.jpg`,
  },
  {
    id: 4,
    name: "Apollonia",
    number_of_activities: 196,
    rating: 4.5,
    image: `${process.env.PUBLIC_URL}/images/apollonia.jpg`,
  },
  {
    id: 5,
    name: "Berat",
    number_of_activities: 234,
    rating: 4.5,
    image: `${process.env.PUBLIC_URL}/images/berat.jpg`,
  },
];

export const topTours = [
  {
    id: 1,
    name: "Japan",
    location: "Japan",
    number_of_places: 10,
    image: `${process.env.PUBLIC_URL}/images/place (8).jpg`,
  },
  {
    id: 2,
    name: "Bali",
    location: "Indonesia",
    number_of_places: 11,
    image: `${process.env.PUBLIC_URL}/images/place (21).jpg`,
  },
  {
    id: 3,
    name: "Barcelona",
    location: "Spain",
    number_of_places: 56,
    image: `${process.env.PUBLIC_URL}/images/place (19).jpg`,
  },
];

export const speedBoatTours = [
  {
    id: 1,
    name: "Tour in the Bay of Saranda",
    price: "45",
    duration: "1 hour",
    rating: "4.7",
    number_of_reviews: 92,
    image: `${process.env.PUBLIC_URL}/images/boat1.jpg`,
    description:
      "Explore the beauties of Saranda on an unforgettable boat trip.",
  },
  {
    id: 2,
    name: "Ksamil Guide - Blue Eye",
    price: "70",
    duration: "2 hours",
    rating: "4.9",
    number_of_reviews: 124,
    image: `${process.env.PUBLIC_URL}/images/boat2.jpg`,
    description:
      "Boat tour from Ksamil to the Blue Eye with stops for photos and swimming.",
  },
  {
    id: 3,
    name: "Exploring Kakomae Bay",
    price: "65",
    duration: "1.5 hours",
    rating: "4.8",
    number_of_reviews: 88,
    image: `${process.env.PUBLIC_URL}/images/boat3.jpg`,
    description:
      "A fast and exciting journey to one of the most beautiful bays.",
  },
];
export const explore = [
  {
    id: 1,
    name: "Home in Vloga Durrës",
    distance: "1.2km",
    address: "Volga Durrës",
    rooms_available: 12,
    rating: 4.8,
    number_of_reviews: 147,
    price: "100",
    image: `${process.env.PUBLIC_URL}/images/place (16).jpg`,
  },
  {
    id: 2,
    name: "Guide to the Blue Eye",
    distance: "300km",
    address: "Syri i Kaltër",
    rooms_available: 12,
    rating: 4.8,
    number_of_reviews: 147,
    price: "55",
    image: `${process.env.PUBLIC_URL}/images/syri-i-kalter.jpg`,
  },
  {
    id: 3,
    name: "Horse Riding Guide",
    distance: "30km",
    address: "Sektor Rinia",
    rooms_available: 12,
    rating: 4.8,
    number_of_reviews: 147,
    price: "210.00",
    image: `${process.env.PUBLIC_URL}/images/horse-ride.jpg`,
  },
  {
    id: 4,
    name: "4 Wheeler Motorcycle Guide",
    distance: "40km",
    address: "Durrës",
    rooms_available: 12,
    rating: 4.8,
    number_of_reviews: 147,
    price: "130",
    image: `${process.env.PUBLIC_URL}/images/motorra-4goma.jpeg`,
  },
  {
    id: 5,
    name: "Guide to Vlora",
    distance: "200km",
    address: "Vlora City",
    rooms_available: 12,
    rating: 4.8,
    number_of_reviews: 147,
    price: "40",
    image: `${process.env.PUBLIC_URL}/images/vlore.jpg`,
  },
  {
    id: 6,
    name: "Guide to Berat",
    distance: "170km",
    address: "Berat City",
    rooms_available: 12,
    rating: 4.8,
    number_of_reviews: 147,
    price: "40",
    image: `${process.env.PUBLIC_URL}/images/berat.jpg`,
  },
  {
    id: 7,
    name: "Guide to Apollonia",
    distance: "37km",
    address: "Fier City",
    rooms_available: 12,
    rating: 4.8,
    number_of_reviews: 147,
    price: "35",
    image: `${process.env.PUBLIC_URL}/images/apollonia.jpg`,
  },
];
export const packagesData = [
  {
    id: "1",
    title: "Home in Vloga Durrës",
    location: "Volga Durrës",
    images: [
      `${process.env.PUBLIC_URL}/images/place (16).jpg`,
      "/images/hotel (2).jpg",
      "/images/hotel (3).jpg",
      "/images/hotel (4).jpg",
    ],
    rating: 4.5,
    tags: ["Good", "Popular"],
    type: "house",
    description:
      "A cozy beach house in Durrës with 2 bedrooms, sea view, and modern amenities. Perfect for families or small groups looking for a relaxing stay by the Adriatic coast.",
    features: [
      "2 Bedrooms",
      "Sea View",
      "Free WiFi",
      "Breakfast included",
      "Air Conditioning",
      "Private Parking",
    ],
    price: "100",
    reviews: 147,
  },
  {
    id: "2",
    title: "Guide to the Blue Eye",
    location: "Sarandë, Albania",
    images: [
      `${process.env.PUBLIC_URL}/images/syri-i-kalter.jpg`,
      "/images/syri2.jpg",
      "/images/syri3.jpg",
      "/images/syri4.jpg",
    ],
    rating: 4.7,
    tags: ["Nature", "Top Rated"],
    type: "tour",
    description:
      "Explore the magical Blue Eye with a local guide. Discover the crystal-clear spring, enjoy a scenic walk, and learn about the area's unique ecosystem.",
    features: [
      "Guided Tour",
      "Nature Walk",
      "Photo Stops",
      "Local Guide",
      "Entrance Ticket Included",
    ],
    price: "55",
    reviews: 124,
  },
  {
    id: "3",
    title: "Skaf në Ksamil",
    location: "Ksamil, Albania",
    images: [
      `${process.env.PUBLIC_URL}/images/speedboat1.jpg`,
      "/images/ksamil2.jpg",
      "/images/ksamil3.jpg",
      "/images/ksamil4.jpg",
    ],
    rating: 4.8,
    tags: ["Boat", "Adventurous"],
    type: "boat",
    description:
      "A thrilling speedboat tour through Ksamil Islands. Swim, sunbathe, and explore hidden beaches with an experienced skipper.",
    features: [
      "Speedboat Tour",
      "Island Hopping",
      "Swimming Stops",
      "Experienced Skipper",
      "Life Jackets Provided",
    ],
    price: "70",
    reviews: 88,
  },
  {
    id: "4",
    title: "Guide ne Apolloni",
    location: "Fier City",
    images: [
      `${process.env.PUBLIC_URL}/images/apollonia.jpg`,
      "/images/ksamil2.jpg",
      "/images/ksamil3.jpg",
      "/images/ksamil4.jpg",
    ],
    rating: 4.8,
    tags: ["History", "Culture"],
    type: "tour",
    description:
      "Discover the ancient ruins of Apollonia with a professional guide. Walk through archaeological sites and learn about Greek and Roman history.",
    features: [
      "Guided Archaeological Tour",
      "Museum Visit",
      "Ancient Ruins",
      "Professional Guide",
      "Entrance Fees Included",
    ],
    price: "35",
    reviews: 92,
  },
  {
    id: "5",
    title: "Horse Riding Guide",
    location: "Sektor Rinia",
    images: [
      `${process.env.PUBLIC_URL}/images/horse-ride.jpg`,
      "/images/ksamil2.jpg",
      "/images/ksamil3.jpg",
      "/images/ksamil4.jpg",
    ],
    rating: 4.8,
    tags: ["Adventure", "Nature"],
    type: "activity",
    description:
      "Enjoy a guided horse riding experience along scenic trails. Suitable for beginners and experienced riders, with safety equipment provided.",
    features: [
      "Guided Horse Riding",
      "Scenic Trails",
      "Safety Equipment",
      "Suitable for All Levels",
      "Photo Opportunities",
    ],
    price: "45",
    reviews: 73,
  },
  {
    id: "6",
    title: "4 Wheeler Motorcycle Guide",
    location: "Durrës",
    images: [
      `${process.env.PUBLIC_URL}/images/motorra-4goma.jpeg`,
      "/images/ksamil2.jpg",
      "/images/ksamil3.jpg",
      "/images/ksamil4.jpg",
    ],
    rating: 4.8,
    tags: ["Adventure", "Off-road"],
    type: "activity",
    description:
      "Experience the thrill of off-road adventure with a 4-wheeler motorcycle guide. Explore rugged terrain and enjoy breathtaking views.",
    features: [
      "4-Wheeler Motorcycle",
      "Off-road Adventure",
      "Safety Gear Provided",
      "Guided Tour",
      "Suitable for Beginners",
    ],
    price: "60",
    reviews: 65,
  },
];
export const tourPackages = [
  {
    id: 1,
    name: "Blue Eye",
    location: "Saranda, Albania",
    rating: "4.6",
    number_of_reviews: 147,
    price: "55",
    image: `${process.env.PUBLIC_URL}/images/syri-i-kalter.jpg`,
    gallery: [
      `${process.env.PUBLIC_URL}/images/syri-i-kalter2.jpg`,
      `${process.env.PUBLIC_URL}/images/syri-i-kalter3.jpg`,
      `${process.env.PUBLIC_URL}/images/syri-i-kalter4.jpg`,
    ],
    video: "https://www.youtube.com/embed/IRi_nMZAlJg",
    description:
      "The Blue Eye is a famous natural spring in Albania, known for its crystal-clear deep blue water and surrounding green landscape. A perfect destination for nature lovers.",
    itinerary: [
      { time: "08:00", activity: "Departure from Saranda" },
      { time: "09:00", activity: "Arrival at Blue Eye Natural Park" },
      { time: "09:30", activity: "Guided tour and photo session" },
      { time: "11:00", activity: "Free time to explore or relax" },
      { time: "13:00", activity: "Return trip to Saranda" },
    ],
    included: [
      "Round trip transportation",
      "Professional guide",
      "Entrance ticket to the park",
    ],
    notIncluded: ["Meals and drinks", "Personal expenses", "Travel insurance"],
  },
  {
    id: 2,
    name: "Vlorë City Tour",
    location: "Vlorë, Albania",
    rating: "4.6",
    number_of_reviews: 147,
    price: "40",
    image: `${process.env.PUBLIC_URL}/images/vlore.jpg`,
    gallery: [
      `${process.env.PUBLIC_URL}/images/vlore.jpg`,
      `${process.env.PUBLIC_URL}/images/vlore-2.jpg`,
      `${process.env.PUBLIC_URL}/images/vlore-3.jpg`,
    ],
    description:
      "Vlorë is a beautiful coastal city where the Adriatic and Ionian Seas meet. Known for its seafront promenade, ancient ruins, and historical sites.",
    itinerary: [
      { time: "09:00", activity: "Meeting point in city center" },
      { time: "09:30", activity: "Visit to Independence Monument" },
      { time: "10:30", activity: "Walk along Lungomare Boulevard" },
      { time: "12:00", activity: "Visit to Zvernec Monastery" },
      { time: "13:30", activity: "End of tour" },
    ],
    included: ["Walking guide", "Transportation to Zvernec", "Entrance fees"],
    notIncluded: ["Lunch", "Tips", "Personal shopping"],
  },
  {
    id: 3,
    name: "Berat – City of a Thousand Windows",
    location: "Berat, Albania",
    rating: "4.6",
    number_of_reviews: 147,
    price: "40",
    image: `${process.env.PUBLIC_URL}/images/berat.jpg`,
    gallery: [
      `${process.env.PUBLIC_URL}/images/berat.jpg`,
      `${process.env.PUBLIC_URL}/images/berat-2.jpg`,
      `${process.env.PUBLIC_URL}/images/berat-3.jpg`,
    ],
    description:
      "A UNESCO World Heritage Site, Berat is known for its unique white Ottoman houses and historical landmarks like the Berat Castle and Onufri Museum.",
    itinerary: [
      { time: "07:30", activity: "Departure from Tirana" },
      { time: "10:00", activity: "Arrival in Berat and coffee break" },
      { time: "10:30", activity: "Guided tour of Berat Castle and old town" },
      { time: "12:30", activity: "Visit Onufri Iconography Museum" },
      { time: "14:00", activity: "Free time for lunch and exploration" },
      { time: "16:00", activity: "Return to Tirana" },
    ],
    included: [
      "Transport by air-conditioned vehicle",
      "Licensed guide",
      "Entrance to museums",
    ],
    notIncluded: ["Lunch", "Drinks", "Souvenirs"],
  },
  {
    id: 4,
    name: "Apollonia Archaeological Park",
    location: "Fier, Albania",
    rating: "4.6",
    number_of_reviews: 147,
    price: "35",
    image: `${process.env.PUBLIC_URL}/images/apollonia.jpg`,
    gallery: [
      `${process.env.PUBLIC_URL}/images/apollonia.jpg`,
      `${process.env.PUBLIC_URL}/images/apollonia-2.jpg`,
      `${process.env.PUBLIC_URL}/images/apollonia-3.jpg`,
    ],
    description:
      "Apollonia is one of Albania’s most important archaeological sites, offering a deep dive into ancient Greek and Roman culture amidst scenic countryside views.",
    itinerary: [
      { time: "08:30", activity: "Departure from Fier" },
      { time: "09:00", activity: "Arrival at Apollonia" },
      { time: "09:15", activity: "Guided tour through ruins and museum" },
      { time: "11:00", activity: "Free time for photos" },
      { time: "12:00", activity: "Return to Fier" },
    ],
    included: ["Transport", "Entrance ticket", "Tour guide"],
    notIncluded: ["Food & beverages", "Optional souvenirs"],
  },
  {
    id: 5,
    name: "Korçë & Voskopoja Village",
    location: "Korçë & Voskopoja, Albania",
    rating: "4.6",
    number_of_reviews: 147,
    price: "45",
    image: `${process.env.PUBLIC_URL}/images/Korce-Voskopoje.jpg`,
    gallery: [
      `${process.env.PUBLIC_URL}/images/Korce-Voskopoje.jpg`,
      `${process.env.PUBLIC_URL}/images/korce-2.jpg`,
      `${process.env.PUBLIC_URL}/images/korce-3.jpg`,
    ],
    description:
      "Explore the cultural capital of Albania and the historic mountain village of Voskopoja, famous for its old Orthodox churches and serene landscapes.",
    itinerary: [
      { time: "07:00", activity: "Departure from Tirana" },
      { time: "11:00", activity: "Arrival in Korçë and walking tour" },
      { time: "13:00", activity: "Departure to Voskopoja" },
      { time: "14:00", activity: "Church visit and mountain lunch" },
      { time: "16:30", activity: "Return journey" },
    ],
    included: ["Transport", "Local guide", "Entrance fees"],
    notIncluded: ["Lunch", "Drinks", "Insurance"],
  },
  {
    id: 6,
    name: "Shala River Adventure",
    location: "Shkoder & Shala River, Albania",
    rating: "4.6",
    number_of_reviews: 147,
    price: "35",
    image: `${process.env.PUBLIC_URL}/images/lumi-i-shales.jpg`,
    gallery: [
      `${process.env.PUBLIC_URL}/images/lumi-i-shales.jpg`,
      `${process.env.PUBLIC_URL}/images/shala-2.jpg`,
      `${process.env.PUBLIC_URL}/images/shala-3.jpg`,
    ],
    description:
      "Nicknamed 'Albanian Thailand', the Shala River is one of the most exotic spots in Albania. Reachable only by boat, this river canyon offers breathtaking nature.",
    itinerary: [
      { time: "06:00", activity: "Departure from Shkoder" },
      { time: "08:00", activity: "Boat ride on Komani Lake" },
      { time: "10:00", activity: "Arrival at Shala River beach" },
      { time: "10:30", activity: "Hiking, swimming and lunch break" },
      { time: "15:00", activity: "Return by boat" },
      { time: "17:00", activity: "Return to Shkoder" },
    ],
    included: ["Transport", "Boat ride", "Guide"],
    notIncluded: ["Lunch", "Snacks", "Swimwear & towels"],
  },
];

export const travelPassions = [
  {
    id: 1,
    name: "Korçë and Voskopoja",
    rating: "4.6",
    number_of_reviews: 147,
    image: `${process.env.PUBLIC_URL}/images/Korce-Voskopoje.jpg`,
  },
  {
    id: 2,
    name: "Speedboat Trip",
    rating: "4.6",
    number_of_reviews: 147,
    image: `${process.env.PUBLIC_URL}/images/gomone-durres.png`,
  },
  {
    id: 3,
    name: "Horse Riding Guide",
    rating: "4.6",
    number_of_reviews: 147,
    image: `${process.env.PUBLIC_URL}/images/kuaj-guide.jpg`,
  },
  {
    id: 4,
    name: "4-Wheeler Motorcycle Guide",
    rating: "4.6",
    number_of_reviews: 147,
    image: `${process.env.PUBLIC_URL}/images/motorra-4goma.jpeg`,
  },
];

export const rooms = [
  {
    id: 1,
    name: "economy room, annex building",
    conditions: ["Non-refundable", "Free Wifi", "Breakfast for 2 people"],
    price: 125,
  },
  {
    id: 2,
    name: "double room",
    conditions: ["Non-refundable", "Free Wifi", "Breakfast for 3 people"],
    price: 165,
  },
  {
    id: 3,
    name: "premium bedroom",
    conditions: [
      "Air Conditioning",
      "Breakfast for 3 people",
      "Flat-Screen TV",
      "Free Wifi",
    ],
    price: 323,
  },
];

export const related = [
  {
    id: 1,
    name: "green house guest house",
    number_of_reviews: 167,
    rating: 4.5,
    price: 247,
    image: `${process.env.PUBLIC_URL}/images/place (25).jpg`,
  },
  {
    id: 2,
    name: "green house guest house",
    number_of_reviews: 167,
    rating: 4.5,
    price: 247,
    image: `${process.env.PUBLIC_URL}/images/place (26).jpg`,
  },
  {
    id: 3,
    name: "green house guest house",
    number_of_reviews: 167,
    rating: 4.5,
    price: 247,
    image: `${process.env.PUBLIC_URL}/images/place (27).jpg`,
  },
];

export const brands = [
  {
    id: 1,
    image: `${process.env.PUBLIC_URL}/images/cars/logo1.png`,
  },
  {
    id: 2,
    image: `${process.env.PUBLIC_URL}/images/cars/logo2.png`,
  },
  {
    id: 3,
    image: `${process.env.PUBLIC_URL}/images/cars/logo3.png`,
  },
  {
    id: 4,
    image: `${process.env.PUBLIC_URL}/images/cars/logo4.png`,
  },
  {
    id: 5,
    image: `${process.env.PUBLIC_URL}/images/cars/logo5.png`,
  },
  {
    id: 6,
    image: `${process.env.PUBLIC_URL}/images/cars/logo6.png`,
  },
];
